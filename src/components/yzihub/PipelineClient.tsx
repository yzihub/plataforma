...
const STAGE_ACTIONS: Record<string, CrmAction[]> = {
  new:         ["contact"],
  contacted:   ["schedule"],
  qualified:   ["schedule"],
...
const STATUS_BADGE: Record<string, { color: BadgeColor; label: string }> = {
  ...
  negotiation: { color: "primary", label: "📋 Contrato" },
...
const ORIGEM_COLORS: Record<string, string> = {
  ...
  LinkedIn:      "bg-sky-100 text-sky-700 dark:bg-sky-500/15 dark:text-sky-400",
  "Google Ads":  "bg-yellow-100 text-yellow-700 dark:bg-yellow-500/15 dark:text-yellow-400",
  TikTok:        "bg-fuchsia-100 text-fuchsia-700 dark:bg-fuchsia-500/15 dark:text-fuchsia-400",
};
...
// Map position → status (para DnD — cobre os dois pipelines)
...
function Avatar({ lead }: { lead: Lead }) {
  return (
    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white shrink-0 ${avatarColor(lead.id)}`}>
...
function PipelineLeadCard({
  ...
  const actions = (STAGE_ACTIONS[lead.status] ?? []) as CrmAction[];
...
function PipelineColumn({
  ...
  return (
    ...
    >
      ...
      <div className={`flex flex-col gap-2.5 p-2.5 flex-1 overflow-y-auto max-h-[calc(100vh-260px)] min-h-[80px] transition-colors ${
...
export default function PipelineClient({
  ...
  const stages = [...initialStages].sort((a, b) => a.position - b.position);
  ...
  const handleDrop = useCallback((e: React.DragEvent, stageId: string) => {
    ...
    const newStatus = POSITION_STATUS[targetStage.position] ?? "new";

    setLeads((prev) =>
      prev.map((l) =>
        l.id === leadId
          ? { ...l, stage_id: stageId, status: newStatus as Lead["status"], last_action_at: new Date().toISOString() }
          : l
      )
    );
